input='''Game 1: 8 green; 5 green, 6 blue, 1 red; 2 green, 1 blue, 4 red; 10 green, 1 red, 2 blue; 2 blue, 3 red
...
def main(txt, part1=False):
    ...
    for line in txt.split('\n'):
        ...
        game_txt, game_picks = line.split(":")
        picks = game_picks.strip().split(";")
        ...
        for pick in picks:
            ...
            for cube in cubes:
                ...
                if part1 and load[color] < int(count):
                    print(count, color)
                    skip = True
        ...
        power = int(minimum['red'])*int(minimum['blue'])*int(minimum['green'])
        powers.append(power)

        id = int(game_txt.strip().split(" ")[-1].strip())
        total.append(id)
